...
public class FormPropertyParser extends BaseChildElementParser {
  ...
  public void parseChildElement(XmlPullParser xtr, BaseElement parentElement, BpmnModel model) throws Exception {
    ...
    try {
      ...
      while (readyWithFormProperty == false && eventType != XmlPullParser.END_DOCUMENT) {
        if (eventType == XmlPullParser.START_TAG && ELEMENT_VALUE.equalsIgnoreCase(xtr.getName())) {
        ...
        } else if (xtr.getEventType() == XmlPullParser.END_TAG && getElementName().equalsIgnoreCase(xtr.getName())) {
