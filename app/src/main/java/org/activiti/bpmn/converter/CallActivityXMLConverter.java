...
public class CallActivityXMLConverter extends BaseBpmnXMLConverter {
  ...
  protected void writeAdditionalAttributes(BaseElement element, BpmnModel model, XmlSerializer xtw) throws Exception {
    ...
    if (StringUtils.isNotEmpty(callActivity.getCalledElement())) {
      xtw.attribute("", ATTRIBUTE_CALL_ACTIVITY_CALLEDELEMENT, callActivity.getCalledElement());
    }
  }
  ...
  private boolean writeIOParameters(String elementName, List<IOParameter> parameterList, boolean didWriteExtensionStartElement, XmlSerializer xtw) throws Exception {
    ...
    for (IOParameter ioParameter : parameterList) {
      ...
      xtw.startTag(ACTIVITI_EXTENSIONS_NAMESPACE, elementName);
      ...
      xtw.endTag(ACTIVITI_EXTENSIONS_NAMESPACE, elementName);
    }
