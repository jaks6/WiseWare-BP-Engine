    ...
    if (!userTask.getCustomProperties().isEmpty()) {
      for (CustomProperty customProperty : userTask.getCustomProperties()) {
        ...
        if (didWriteExtensionStartElement == false) {
          ...
          xtw.startTag(ACTIVITI_EXTENSIONS_NAMESPACE, customProperty.getName());
          xtw.cdsect(customProperty.getSimpleValue());
          xtw.endTag(ACTIVITI_EXTENSIONS_NAMESPACE, customProperty.getName());
...
//        xtw.writeEndElement();
  ...
  protected boolean writeCustomIdentities(BaseElement element, boolean didWriteExtensionStartElement, XmlSerializer xtw) throws Exception {
	  UserTask userTask = (UserTask) element;
  ...
  protected void writeCustomIdentities(UserTask userTask,String identityType, Set<String> users, Set<String> groups, XmlSerializer xtw) throws Exception {


      xtw.setPrefix(ACTIVITI_EXTENSIONS_PREFIX, ACTIVITI_EXTENSIONS_NAMESPACE);
      xtw.startTag(ACTIVITI_EXTENSIONS_NAMESPACE, ELEMENT_CUSTOM_RESOURCE);
...
//    xtw.writeCharacters(delimitedString);
      ...
      xtw.startTag(ACTIVITI_EXTENSIONS_NAMESPACE, ELEMENT_RESOURCE_ASSIGNMENT);
      xtw.startTag(ACTIVITI_EXTENSIONS_NAMESPACE, ELEMENT_FORMAL_EXPRESSION);

      xtw.text(delimitedString);


      xtw.endTag(ACTIVITI_EXTENSIONS_NAMESPACE, ELEMENT_FORMAL_EXPRESSION);
//    xtw.writeEndElement(); // End ELEMENT_FORMAL_EXPRESSION
      xtw.endTag(ACTIVITI_EXTENSIONS_NAMESPACE, ELEMENT_RESOURCE_ASSIGNMENT);
//    xtw.writeEndElement(); // End ELEMENT_RESOURCE_ASSIGNMENT
    
//    xtw.writeEndElement(); // End ELEMENT_CUSTOM_RESOURCE
    xtw.endTag(ACTIVITI_EXTENSIONS_NAMESPACE, ELEMENT_CUSTOM_RESOURCE);
  }
    ...
    public void parseChildElement(XmlPullParser xtr, BaseElement parentElement, BpmnModel model) throws Exception {
      ...
      if (StringUtils.isNotEmpty(resourceElement) && ELEMENT_RESOURCE_ASSIGNMENT.equals(resourceElement)) {
        ...
        if (StringUtils.isNotEmpty(expression) && ELEMENT_FORMAL_EXPRESSION.equals(expression)) {
          ((UserTask) parentElement).setAssignee(xtr.getText());
        }
      }
    }
  }
    ...
    public void parseChildElement(XmlPullParser xtr, BaseElement parentElement, BpmnModel model) throws Exception {
      ...
      if (StringUtils.isNotEmpty(resourceElement) && ELEMENT_RESOURCE_ASSIGNMENT.equals(resourceElement)) {
        ...
        if (StringUtils.isNotEmpty(expression) && ELEMENT_FORMAL_EXPRESSION.equals(expression)) {
          ...
          List<String> assignmentList = CommaSplitter.splitCommas(xtr.getText());
