...
public class DataStoreReferenceXMLConverter extends BaseBpmnXMLConverter {
  ...
  protected void writeAdditionalAttributes(BaseElement element, BpmnModel model, XmlSerializer xtw) throws Exception {
    ...
    if (StringUtils.isNotEmpty(dataStoreRef.getDataStoreRef())) {
      ...
      xtw.attribute("",ATTRIBUTE_DATA_STORE_REF, dataStoreRef.getDataStoreRef());
    }
    
    if (StringUtils.isNotEmpty(dataStoreRef.getItemSubjectRef())) {
      xtw.attribute("",ATTRIBUTE_ITEM_SUBJECT_REF, dataStoreRef.getItemSubjectRef());
    }
  }
  ...
  protected void writeAdditionalChildElements(BaseElement element, BpmnModel model, XmlSerializer xtw) throws Exception {
    ...
    if (StringUtils.isNotEmpty(dataStoreRef.getDataState())) {
      ...
      xtw.text(dataStoreRef.getDataState());
